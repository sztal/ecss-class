...
\begin{frame}
    ...
    \only<3,5>{
        ...
        \begin{enumerate}
            ...
            \item<5> Double click on R-3.6.1.pkg and follow the default installation
            \item<5> Double click on XQuartz\_2.7.11.dmg and follow the default installation
        \end{enumerate}
    }
    \only<4>{
    ...
    \only<6,8>{
        ...
        \begin{enumerate}
            ...
            \item<8> Double click on R-3.6.1-win.exe and follow the default installation
            \item<8> Double click on Rtools35.exe and follow the default installation
        \end{enumerate}
    }
    \only<7>{
...
\begin{frame}
    ...
    \only<2>{
        ...
        \emph{RStudio} is an integrated development environment (IDE) for R. In other words it is a program which allows you to write and execute R scripts in more than convinient way.
    }
    \only<3,5>{
        \frametitle{How to install RStudio?}
        To install RStudio you need to \alert{have R installed} and follow these six easy steps:
        \begin{enumerate}
            \item<3,5> Visit \textcolor{blue}{\href{https://rstudio.com/products/rstudio/download/}{www.rstudio.com}}
            \item<3,5> Click download
            \item<3,5> Choose RStudio Desktop (the free version)
            \item<5> Download version appropriate for your operating system (Windows or macOS)
            \item<5> When the file is downloaded double click on it and install RStudio in the default location
        \end{enumerate}
    }
    \only<4>{
        \includegraphics[width = \framewidth]{png/rstudio_download}
    }
    \only<6>{
        \frametitle{Voila!}
...
\begin{frame}
    ...
    \only<+>{
        ...
        \begin{definition}
            \emph{Python} is a programming language and the Python interpreter software that reads the source code and performs its instructions. There are two generations of Python: Python 2.x and Python 3.x. \alert{We will use only Python 3.x.}
        \end{definition}
    }
    \only<+>{
        \frametitle{Python}
...
\begin{frame}
    \only<+>{
        \includegraphics[width = \framewidth]{png/colab.png}
    }
    \only<+>{
...
\begin{frame}[fragile]
    ...
    \only<2>{
        ...
        \begin{definition}
            \emph{Anaconda} is a free and open-source distribution of Python and R programming langugaes for scientific mputing that aims to simplify package management and deployment. It comes with more than 1500 packages installed and conda package and virtual environment manager.
        \end{definition}
    }
    \only<3>{
    ...
    \only<4,5,7>{
        ...
        Unfortunetly installing Anaconda on the new macOS became a bit tricky especially if you already had Anaconda installed when you updated to macOS Catalina. Anyway, before you go any further (no matter what macOS you have) follow these three steps:
        \begin{enumerate}
            \item<4,5,7> Press CMD and SPACE at the same time
            \item<4,5,7> Type Terminal and press RETURN (ENTER)
            \item<7> Type \mintinline{bash}{xcode-select --install} and press RETURN (ENTER)
        \end{enumerate}
    }
    \only<6>{
    ...
    \only<9,11,13>{
        ...
        \begin{enumerate}
            \item<9,11,13> Visit \textcolor{blue}{\href{https://www.anaconda.com}{www.anaconda.com}}
            \item<9,11,13> Click download and scroll down to choose version
            \item<9,11,13> Download Python 3.7 version 64-Bit Command Line Installer
            \item<11,13> Type \mintinline{bash}{bash ~/Downloads/Anaconda3-2019*.sh} in Terminal and press RETURN (ENTER)
            \item<13> Type \mintinline{bash}{jupyter notebook} in Terminal and press RETURN (ENTER) to open the Jupyter Notebook App
        \end{enumerate}
    }
    \only<10>{
    ...
    \only<15,17>{
        ...
        \begin{enumerate}
            \item<15,17> Visit \textcolor{blue}{\href{https://www.anaconda.com}{www.anaconda.com}}
            \item<15,17> Click download and scroll down to choose version
            \item<15,17> Download Python 3.7 64-Bit Graphical Installer
            \item<15,17> Double click on Anaconda3-2019.10-Windows-x86\_64.exe and follow the default installation
            \item<17> Type \mintinline{bash}{jupyter notebook} in Anaconda Prompt and press ENTER to open Jupyter Notebook
        \end{enumerate}
    }
    \only<16>{
